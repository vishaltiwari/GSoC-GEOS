...
QuadEdgeSubdivision::getVoronoiCellPolygons(std::vector<geom::Geometry *> &cells ,const geom::GeometryFactory& geomFact)
{
	...
	{
		QuadEdge *qe = *it;
		cells.push_back(/*static_cast<Geometry*>*/getVoronoiCellPolygon(qe,geomFact));
	}
}

/*Geometry* QuadEdgeSubdivision::getVoronoiCellPolygon(QuadEdge* qe ,const geom::GeomtryFactory& geomFact)
{
	...
	//For addAll
	geom::CoordinateList::iterator iter = coordList.begin();
	for(std::vector::iterator it=cellPts.begin() ; it!=cellPts.end() ; ++it)
	{
		coordList.insert(iter,*it,false);
		++iter;
	}
	//for checking close ring
	if(coordList.size() > 0)
	{
		coordList.insert(coordList.end(),new Coordinate(*(coordList.begin())),false);
	}
	if(coodList.size() < 4)
	{
		cout << coorList;
	...
	(*pts_seq).add(pts,true);
	geom::Polygon *cellPoly = geomFact.createPolygon(geomFact.createLinearRing(pts_seq),NULL);

	Vertex v = startQE.orig();
	(*cellPoly).setUserData(v.getCoordinate());
	
	return static_cast<Geometry*>(cellPoly);
}*/

QuadEdgeList* QuadEdgeSubdivision::getVertexUniqueEdges(bool includeFrame)
{
	QuadEdgeList *edges;
	std::unordered_set<Vertex> visitedVertices;
	for(QuadEdgeSubdivisiob::QuadEdgeList::iterator it=quadEdges.begin() ; it!=quadEdges.end() ; ++it)
	{
		QuadEdge *qe = (QuadEdge*)(*it);
		Vertex v = (*qe).orig();

		std::unordered_set<Vertex>::iterator got = visitedVertices.find(v);
		...
		std::unordered_set<Vertex>::iterator got = visitedVertices.find(vd);
		if(got == visitedVertices.end()){
